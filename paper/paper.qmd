...
The present research aims to complement these two techniques, by using high resolution mapping of human activity in the dry valleys to extrapolate how scientists navigate in these landscapes, or could potentially navigate in these landscapes.
...
Having found these connections we then include the shortest paths from the original hiking network in both directions between each pair of connected sites in the minimum spanning tree.
...
For example, in @fig-approx-minimum-arborescence, it might be preferable to 'disconnect' sites 3 and 13, and instead connect the basecamp (site 4) to site 13 to avoid traversal of the valley floor in this area in more than one location.
Refinement of plans might also lead to only one of each of pair of connections between sites being used, or in some places due to ground conditions the preference might be to allow scientists to make their own way between sites to disperse the impact of trampling in those areas.
...
ggplot() +
  ...
  geom_sf(data = origin_vs, aes(size = Importance, colour = is_camp), pch = 1) +
  scale_colour_manual(values = c("black", "red")) +
  geom_sf_text(data = origin_vs, aes(label = seq_no)) +
...
Furthermore, our use not of total betweenness, but of radius-limited betweenness rests on an implied model of movement that can only be verified anecdotally. 
A radius-limited approach implies that in traversing such terrain a group of scientists plan their movements based on regularly reassessment of their route at regular intervals.
...
These limitations notwithstanding, our approach yields plausible seeming maps of the overall relative risk of impacts from scientist moving around in these environments, and offers the potential to plan for reducing impact in these vulnerable environments.

### Planning to reduce impact
Our proposal for planning expedition route networks responds to O'Neill et al.'s suggestion that, "long-term environmental impacts in Antarctica can be reduced through careful site remediation and site selection, but perhaps more importantly by taking appropriate action in the project design, planning, and operational stages of the activity" [-@ONeill2013, p. 529].
...
This means that a useful extenstion of least (time) cost movement paths would be to penalise movement based on terrain vulnerability to disturbance, in order to prioritise identification of routes that cross less vulnerable terrain.
